...
Elm.HackerNews.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var FetchTopStories = function (a) {    return {ctor: "FetchTopStories",_0: a};};
   var FetchTopStoriesId = function (a) {    return {ctor: "FetchTopStoriesId",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var itemAddressStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "block"}
                                                         ,{ctor: "_Tuple2",_0: "opacity",_1: ".6"}
                                                         ,{ctor: "_Tuple2",_0: "font-size",_1: ".8em"}
                                                         ,{ctor: "_Tuple2",_0: "color",_1: "black"}
                                                         ,{ctor: "_Tuple2",_0: "text-decoration",_1: "none"}]));
   var itemTitleStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "black"}
                                                       ,{ctor: "_Tuple2",_0: "display",_1: "block"}
                                                       ,{ctor: "_Tuple2",_0: "text-decoration",_1: "none"}]));
   var itemStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "list-style",_1: "none"},{ctor: "_Tuple2",_0: "margin",_1: "1em"}]));
   var itemListStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background-color",_1: "#f6f6ef"}
                                                      ,{ctor: "_Tuple2",_0: "margin",_1: "0"}
                                                      ,{ctor: "_Tuple2",_0: "padding",_1: ".2em"}]));
   var headerStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}
                                                    ,{ctor: "_Tuple2",_0: "margin",_1: "0"}
                                                    ,{ctor: "_Tuple2",_0: "padding",_1: ".2em"}]));
   var viewStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "80%"}
                                                  ,{ctor: "_Tuple2",_0: "background-color",_1: "#ff6600"}
                                                  ,{ctor: "_Tuple2",_0: "margin",_1: "auto"}]));
   var item = function (story) {
      var formatedUrl = A2($Maybe.withDefault,"",$List.head(A2($List.drop,2,A2($String.split,"/",story.url))));
      var addressLink = $Html$Attributes.href(A2($Basics._op["++"],"http://",formatedUrl));
      var titleLink = $Html$Attributes.href(story.url);
      return A2($Html.li,
      _U.list([itemStyle]),
      _U.list([A2($Html.a,_U.list([itemTitleStyle,titleLink]),_U.list([$Html.text(story.title)]))
              ,A2($Html.a,_U.list([itemAddressStyle,addressLink]),_U.list([$Html.text(formatedUrl)]))]));
   };
   var itemList = function (model) {    var items = A2($List.map,item,model);return A2($Html.ul,_U.list([itemListStyle]),items);};
   var header = A2($Html.h1,_U.list([headerStyle]),_U.list([$Html.text("Hacker News")]));
   var view = F2(function (address,model) {    return A2($Html.div,_U.list([viewStyle]),_U.list([header,itemList(model)]));});
   var initialModel = _U.list([]);
   var Story = F2(function (a,b) {    return {title: a,url: b};});
   var formatStory = function (story) {
      return A2(Story,story.title,A2($Maybe.withDefault,"",$List.head(A2($List.drop,2,A2($String.split,"/",story.url)))));
   };
   var storyDecoder = A3($Json$Decode.object2,
   Story,
   A2($Json$Decode._op[":="],"title",$Json$Decode.string),
   A2($Json$Decode._op[":="],"url",$Json$Decode.string));
   var hackerNewsItemEndpoint = "https://hacker-news.firebaseio.com/v0/item/";
   var fetchStory = function (storyId) {
      return A2($Http.get,storyDecoder,A2($Basics._op["++"],hackerNewsItemEndpoint,A2($Basics._op["++"],storyId,".json")));
   };
   var fetchTopStories = function (topStoriesId) {
      return $Effects.task(A2($Task.map,FetchTopStories,$Task.toMaybe($Task.sequence(A2($List.map,fetchStory,A2($List.take,10,topStoriesId))))));
   };
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "NoOp": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "FetchTopStoriesId": return {ctor: "_Tuple2",_0: model,_1: fetchTopStories(A2($Maybe.withDefault,_U.list([]),_p0._0))};
         default: return {ctor: "_Tuple2",_0: A2($Maybe.withDefault,_U.list([]),_p0._0),_1: $Effects.none};}
   });
   var hackerNewsTopStoriesEndpoint = "https://hacker-news.firebaseio.com/v0/topstories.json";
   var fetchTopStoriesId = $Effects.task(A2($Task.map,
   FetchTopStoriesId,
   $Task.toMaybe(A2($Http.get,$Json$Decode.list(A2($Json$Decode.map,$Basics.toString,$Json$Decode.$int)),hackerNewsTopStoriesEndpoint))));
   var init = function (model) {    return {ctor: "_Tuple2",_0: model,_1: fetchTopStoriesId};};
   var app = $StartApp.start({init: init(initialModel),update: update,view: view,inputs: _U.list([])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.HackerNews.values = {_op: _op
                                   ,hackerNewsTopStoriesEndpoint: hackerNewsTopStoriesEndpoint
                                   ,hackerNewsItemEndpoint: hackerNewsItemEndpoint
                                   ,Story: Story
                                   ...
                                   ,itemAddressStyle: itemAddressStyle
                                   ,fetchTopStoriesId: fetchTopStoriesId
                                   ,fetchStory: fetchStory
                                   ,fetchTopStories: fetchTopStories
                                   ,storyDecoder: storyDecoder
                                   ,NoOp: NoOp
                                   ,FetchTopStoriesId: FetchTopStoriesId
                                   ,FetchTopStories: FetchTopStories
                                   ,update: update
